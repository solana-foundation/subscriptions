...
export function identifySubscriptionsInstruction(
  ...
  if (containsBytes(data, getU8Encoder().encode(228), 0)) {
    return SubscriptionsInstruction.EmitEvent;
  }
  throw new SolanaError(
    SOLANA_ERROR__PROGRAM_CLIENTS__FAILED_TO_IDENTIFY_INSTRUCTION,
    { instructionData: data, programName: "subscriptions" },
  );
}

export type ParsedSubscriptionsInstruction<
  TProgram extends string = "EPEUTog1kptYkthDJF6MuB1aM4aDAwHYwoF32Rzv5rqg",
> =
  | ({
  ...
  | ({
      instructionType: SubscriptionsInstruction.EmitEvent;
...
export function parseSubscriptionsInstruction<TProgram extends string>(
  ...
  switch (instructionType) {
    ...
    case SubscriptionsInstruction.EmitEvent: {
      assertIsInstructionWithAccounts(instruction);
      return {
        instructionType: SubscriptionsInstruction.EmitEvent,
...
export type SubscriptionsPluginInstructions = {
  ...
  ) => ReturnType<typeof getInitSubscriptionAuthorityInstruction> &
    SelfPlanAndSendFunctions;
  createFixedDelegation: (
    input: CreateFixedDelegationInput,
  ) => ReturnType<typeof getCreateFixedDelegationInstruction> &
    SelfPlanAndSendFunctions;
  createRecurringDelegation: (
    input: CreateRecurringDelegationInput,
  ) => ReturnType<typeof getCreateRecurringDelegationInstruction> &
    SelfPlanAndSendFunctions;
  revokeDelegation: (
  ...
  ) => ReturnType<typeof getTransferFixedInstructionAsync> &
    SelfPlanAndSendFunctions;
  transferRecurring: (
    input: TransferRecurringAsyncInput,
  ) => ReturnType<typeof getTransferRecurringInstructionAsync> &
    SelfPlanAndSendFunctions;
  closeSubscriptionAuthority: (
    input: CloseSubscriptionAuthorityInput,
  ) => ReturnType<typeof getCloseSubscriptionAuthorityInstruction> &
    SelfPlanAndSendFunctions;
  createPlan: (
    input: CreatePlanInput,
  ) => ReturnType<typeof getCreatePlanInstruction> & SelfPlanAndSendFunctions;
  updatePlan: (
    input: UpdatePlanInput,
  ) => ReturnType<typeof getUpdatePlanInstruction> & SelfPlanAndSendFunctions;
  deletePlan: (
    input: DeletePlanInput,
  ) => ReturnType<typeof getDeletePlanInstruction> & SelfPlanAndSendFunctions;
  transferSubscription: (
    input: TransferSubscriptionAsyncInput,
  ) => ReturnType<typeof getTransferSubscriptionInstructionAsync> &
    SelfPlanAndSendFunctions;
  subscribe: (
  ...
  ) => ReturnType<typeof getCancelSubscriptionInstructionAsync> &
    SelfPlanAndSendFunctions;
  emitEvent: (
...
export type SubscriptionsPluginRequirements = ClientWithRpc<
  ...
  return <T extends SubscriptionsPluginRequirements>(client: T) => {
    return {
      ...
      subscriptions: <SubscriptionsPlugin>{
        accounts: {
          fixedDelegation: addSelfFetchFunctions(
            client,
            getFixedDelegationCodec(),
          ),
          plan: addSelfFetchFunctions(client, getPlanCodec()),
          recurringDelegation: addSelfFetchFunctions(
            client,
            getRecurringDelegationCodec(),
          ),
          subscriptionAuthority: addSelfFetchFunctions(
            client,
            getSubscriptionAuthorityCodec(),
          ),
          subscriptionDelegation: addSelfFetchFunctions(
            client,
            getSubscriptionDelegationCodec(),
          ),
        },
        instructions: {
          initSubscriptionAuthority: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getInitSubscriptionAuthorityInstruction(input),
            ),
          createFixedDelegation: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getCreateFixedDelegationInstruction(input),
            ),
          createRecurringDelegation: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getCreateRecurringDelegationInstruction(input),
            ),
          ...
          transferFixed: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getTransferFixedInstructionAsync(input),
            ),
          transferRecurring: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getTransferRecurringInstructionAsync(input),
            ),
          closeSubscriptionAuthority: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getCloseSubscriptionAuthorityInstruction(input),
            ),
          createPlan: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getCreatePlanInstruction(input),
            ),
          updatePlan: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getUpdatePlanInstruction(input),
            ),
          deletePlan: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getDeletePlanInstruction(input),
            ),
          transferSubscription: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getTransferSubscriptionInstructionAsync(input),
            ),
          subscribe: (input) =>
            addSelfPlanAndSendFunctions(
              client,
          ...
          cancelSubscription: (input) =>
            addSelfPlanAndSendFunctions(
              client,
              getCancelSubscriptionInstructionAsync(input),
            ),
          emitEvent: (input) =>
            addSelfPlanAndSendFunctions(
              client,
