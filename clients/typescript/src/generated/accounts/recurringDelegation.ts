...
export function getRecurringDelegationCodec(): FixedSizeCodec<
  RecurringDelegationArgs,
  RecurringDelegation
> {
  return combineCodec(
...
export async function fetchRecurringDelegation<
...
>(
  ...
  const maybeAccount = await fetchMaybeRecurringDelegation(
    rpc,
    address,
    config,
  );
...
export async function fetchMaybeRecurringDelegation<
  TAddress extends string = string,
>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
...
export async function fetchAllRecurringDelegation(
  ...
  const maybeAccounts = await fetchAllMaybeRecurringDelegation(
    rpc,
    addresses,
    config,
  );
...
export async function fetchAllMaybeRecurringDelegation(
  ...
  return maybeAccounts.map((maybeAccount) =>
    decodeRecurringDelegation(maybeAccount),
  );
}
