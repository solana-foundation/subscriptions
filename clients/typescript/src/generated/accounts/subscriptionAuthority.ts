...
export function getSubscriptionAuthorityCodec(): FixedSizeCodec<
  SubscriptionAuthorityArgs,
...
export async function fetchSubscriptionAuthority<
...
>(
  ...
  const maybeAccount = await fetchMaybeSubscriptionAuthority(
    rpc,
    address,
    config,
  );
...
export async function fetchMaybeSubscriptionAuthority<
  TAddress extends string = string,
>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
...
export async function fetchAllSubscriptionAuthority(
  ...
  const maybeAccounts = await fetchAllMaybeSubscriptionAuthority(
    rpc,
    addresses,
    config,
  );
...
export async function fetchAllMaybeSubscriptionAuthority(
  ...
  return maybeAccounts.map((maybeAccount) =>
    decodeSubscriptionAuthority(maybeAccount),
  );
}
