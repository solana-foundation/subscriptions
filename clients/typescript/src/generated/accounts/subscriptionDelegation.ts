...
export function getSubscriptionDelegationCodec(): FixedSizeCodec<
  SubscriptionDelegationArgs,
...
export async function fetchSubscriptionDelegation<
...
>(
  ...
  const maybeAccount = await fetchMaybeSubscriptionDelegation(
    rpc,
    address,
    config,
  );
...
export async function fetchMaybeSubscriptionDelegation<
  TAddress extends string = string,
>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
...
export async function fetchAllSubscriptionDelegation(
  ...
  const maybeAccounts = await fetchAllMaybeSubscriptionDelegation(
    rpc,
    addresses,
    config,
  );
...
export async function fetchAllMaybeSubscriptionDelegation(
  ...
  return maybeAccounts.map((maybeAccount) =>
    decodeSubscriptionDelegation(maybeAccount),
  );
}
