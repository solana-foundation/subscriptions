...
describe('Subscription Security', () => {
    ...
    test('destination whitelist is enforced', async () => {
        ...
        const allowedAta = await t.createAtaWithBalance(t.tokenMint, allowedReceiver.address, 0n);

        const unauthorizedReceiver = await t.createFundedKeypair();
        const unauthorizedAta = await t.createAtaWithBalance(t.tokenMint, unauthorizedReceiver.address, 0n);
