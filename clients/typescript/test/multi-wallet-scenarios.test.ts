...
describe('Multi-Wallet Scenarios', () => {
    test('multi-subscriber isolation: cancel one does not affect others', async () => {
        ...
        const subscribers = await Promise.all(Array.from({ length: 3 }, () => t.createFundedKeypair()));
        const subscriberAtas = await Promise.all(
            subscribers.map(s => t.createAtaWithBalance(t.tokenMint, s.address, DEFAULT_TEST_BALANCE)),
        );
        ...
        const subAData = (await fetchSubscriptionDelegation(t.rpc, subPdas[0])).data;
        const subBData = (await fetchSubscriptionDelegation(t.rpc, subPdas[1])).data;
    ...
    test('re-init defense: user kill-switch blocks merchant, then recovers', async () => {
        ...
        const trustedAta = await t.createAtaWithBalance(t.tokenMint, trustedDelegatee.address, 0n);
    ...
    test('multi-mint kill-switch isolation', async () => {
        ...
        const userAtaA = await t.createAtaWithBalance(t.tokenMint, t.payerKeypair.address, DEFAULT_TEST_BALANCE);
        const userAtaB = await t.createAtaWithBalance(mintB, t.payerKeypair.address, DEFAULT_TEST_BALANCE);
        ...
        const delegateeAtaA = await t.createAtaWithBalance(t.tokenMint, delegatee.address, 0n);
        const delegateeAtaB = await t.createAtaWithBalance(mintB, delegatee.address, 0n);
