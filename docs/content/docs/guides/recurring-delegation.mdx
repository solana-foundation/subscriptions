...
2. Derive the user's token account, Subscription Authority PDA, and recurring delegation PDA.
3. Initialize the Subscription Authority if it does not exist yet.
4. Create the recurring delegation with the period amount, period length, start time, and expiry.
...
- `amountPerPeriod` is in base units. For a 6-decimal token, `1_000_000` means `1` token.
- The program rejects transfers that exceed the current period's remaining allowance.
