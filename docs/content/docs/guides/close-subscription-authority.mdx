...
Closing the account returns its rent to the user and revokes the token delegate approval owned by the program. If any active delegation still depends on the authority, close those delegations first.
