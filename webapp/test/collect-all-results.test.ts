...
import { createAllPlanPaymentCollectionResult, type ConfirmedPlanTransfer } from '../src/lib/collect-all-results.ts';
...
test('aggregates successful collect-all transfers by plan', () => {
    const result = createAllPlanPaymentCollectionResult(
        ...
        [transfer('plan-a', 'sub-a', 'sig-a', 0), transfer('plan-b', 'sub-b', 'sig-b', 1)],
        ['sig-a', 'sig-b'],
        false,
    );
...
test('marks a plan partial only for its own uncollected transfers', () => {
    const result = createAllPlanPaymentCollectionResult(
        ...
        [transfer('plan-a', 'sub-a-1', 'sig-a-1', 0), transfer('plan-b', 'sub-b', 'sig-b', 0)],
        ['sig-a-1', 'sig-b'],
        true,
    );
