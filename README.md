...
- **The sentinel `init_id` for 1-step signup is slot-scoped, not transaction-scoped.** Passing `UNKNOWN_INIT_ID` as the expected `init_id` on `subscribe`/`create_*_delegation` accepts the authority when its stored `init_id` equals the current slot — the intended use is bundling `init_subscription_authority` in the same transaction so a client need not know `Clock::slot` at signing time. Because the check is slot-granular, a signed sentinel instruction that lands in the same slot as _any_ fresh creation of the same authority PDA (for example a durable-nonce transaction held until the user re-creates a previously closed authority) will bind, even across a close/re-create the user intended as revocation. Idempotent re-init does not refresh `init_id`, so this applies only to first creation in that slot; a standalone sentinel call whose authority was created in an earlier slot fails closed. Pass the real `init_id` when it is known, and use `revokeDelegation`/`cancelSubscription` to stop an unwanted delegation.
